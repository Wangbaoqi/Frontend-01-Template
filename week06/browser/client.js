...
class ChunkedBodyParser {
  constructor() {
    this.WAINING_LENGTH = 0;
    this.WAINING_LENGTH_LINR_END = 1;
    this.READING_TRUCK = 2;
    this.WAINING_NEW_LINE = 3;
    this.WAINING_NEW_LINE_END = 4;
    this.length = 0;
    this.content = [];
    this.isFinished = false;
    this.current = this.WAINING_LENGTH;
  }

  receiveChar(char) {
    if(this.current === this.WAINING_LENGTH) {
      if(char === '\r') {
        if(this.length === 0) {
          this.isFinished = true
        }
        this.current = this.WAINING_LENGTH_LINR_END;
      } else {
        this.length *= 16
        this.length += parseInt(char, 16)
      }
    }else if(this.current === this.WAINING_LENGTH_LINR_END) {
      if(char === '\n') {
        this.current = this.READING_TRUCK;
      } 
    }else if(this.current === this.READING_TRUCK) {
      this.content.push(char)
      this.length --;
      if(this.length === 0) {
        this.current = this.WAINING_NEW_LINE;
      } 
    }else if(this.current === this.WAINING_NEW_LINE) {
      if(char === '\r') {
